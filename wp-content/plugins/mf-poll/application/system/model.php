...
class MFPModel {
    ...
    public function find($key, $equation, $value, $settings = null) {
        ...
        $result = $this->db->get_row("SELECT * FROM {$this->db->prefix}{$this->table_name} WHERE {$key} {$equation} '{$value}' {$settings} LIMIT 1");
    ...
    public function update(array $array, array $where) {
        ...
        foreach ($keys as $key) {
            $update[] = $this->db->prepare("{$key}='%s'" ,$array[$key]);
        }
